# Stage 1 gate: structural self-test over every SlowTree preset + the embedded
# HelloTree default template (mesh non-empty / no NaN / sane AABB / determinism /
# v1 node gating). No byte-level golden — see Source/TreeGen/SlowTree/UPSTREAM_SYNC.md.
#
# Usage:
