...
grew a `Lift` that raised the chord off the surface by `0.07D` — also physically right, since tiles
sit on battens. Worth remembering as a general rule for this module: any sweep laid onto a generated
surface needs an explicit lift. (Phase 6 replaced the tubes entirely and moved that constant into
`TileSection::MinimumLift()`, derived from the channel depth, so it can no longer drift out of step
with the tiles it is meant to clear.)
...
### Phase 6 — the tile skin (瓦面) — **done, verified**

The tiling was rebuilt because it was wrong in kind, not in detail. Up to phase 5 each 瓦垄 was a
separate half-round tube swept on top of a flat boarding surface, so the roof had a *flat interval*
between its ridges — which no Chinese roof has. Rendered close up it read as corrugated steel, and
it cost about **two thirds of the whole building's triangles**, because every tube carried a closed
perimeter including an underside nobody can see.

A real roof is one continuous scalloped skin: 板瓦 pan tiles form concave channels and 筒瓦 barrel
tiles cap the joints between them. So `Source/AncientBuilding/TileSkin.*` generates it as a
displaced surface instead. The caller owns the roof's parameterisation — where the courses go, and
where a hip cuts them off — and hands over one column of points per section sample; the skin owns
the cross-section, the shading and the eave tiles.

`TileSection` is eight samples per course over one pitch, in units of that pitch:

| sample | offset | height | role |
|---|---|---|---|
| 0 | −0.500 | −0.17 | 板瓦 channel bottom, shared with the previous course |
| 1 | −0.155 | 0 | pan edge — crease A |
| 2 | −0.155 | 0 | 筒瓦 base — crease A |
| 3–5 | −0.110 … +0.110 | up to +0.16 | barrel flank, crown, flank |
| 6 | +0.155 | 0 | 筒瓦 base — crease B |
| 7 | +0.155 | 0 | pan edge — crease B |

Three things in that table are load-bearing:

- **The barrel is narrower than its trough**, roughly 1:2. Equal widths give a sine wave; the eye
  needs a round rib against a broad channel to read tile rather than corrugation. This was the
  single biggest visual lever, ahead of channel depth.
- **The crease pairs are coincident in position and differ only in normal.** Their quads are
  degenerate and get dropped, so the hard 筒瓦 edge costs *no triangles* — it is bought entirely
  with `MeshAccumulator::AddQuadSmooth`, which takes a normal per corner.
- **The pan's geometry is a straight ramp but its normals are a parabola's.** Sampling the curve
  properly cost 25% of the skin and only showed on the eave silhouette, which 滴水 covers anyway.
  Smooth normals carry a curved trough over flat geometry perfectly well.

The batten lift now comes from `TileSection::MinimumLift()` — the channel depth plus a clearance —
instead of being a hand-picked `0.07D` at each call site. That constant is what caused the phase-3a
z-fighting; tying it to the section's own depth means it cannot drift out of step again.

**瓦当 and 滴水** are generated by the skin, not the callers, because only it knows where the barrels
and channels landed after displacement and in what frame. They are the cheapest detail on the roof
by a wide margin — `O(courses)`, not `O(courses × rafters)` — while sitting on the one edge of the
roof a player sees from the ground, and they are what turns the section's raw sawtooth into the
fringed drip line a Chinese roof is recognised by. The 滴水 spine needs **three** knots: the tile
continues the roof plane past the eave and then its outer edge turns down. With two it is invisible
except from dead outside and reads as a notch between the 瓦当.

`ETileEaves` says which ends of a column are eaves. Columns run eave-first, so `AtStart` is usual;
卷棚 is `AtBothEnds` because one profile runs eave to eave over the roll; the 歇山 upper tier is
`None` because its lower edge is the 收山 break under a 博脊, not an eave. And when `tile_coverage`
is below 1 the roof is bared *from the eave upward*, so column 0 is partway up the slope and gets
`None` too.

Adding the eave tiles forced a second change: the old continuous 滴水 sweep stood **in front of**
them and hid the lot. It is now a slim 连檐 board, dropped and set back so it reads as the timber
the courses die onto. Same element, honest size.

Two costs worth recording plainly:

- Skin vs tubes, same scene and parameters (歇山, 11×7 m): marginal cost went from **362 to 391
  triangles per course**, about +8%. My prediction that it would be *cheaper* was wrong — the skin
  has 8 cross-section facets where the tube had 6, two of which were its hidden underside. The
  first attempt was +40% until the pan mid-samples came out.
- Eave tiles add **+42%** on a 歇山 hall (18,960 → 26,864). Deliberate: it is the highest-value
  detail per triangle in the whole roof.

On the centralised family the courses converge on the apex, so their pitch shrinks with the plan
radius and taken all the way in the barrels end up narrower than a pixel. `TILE_APEX_CUTOFF` stops
the skin at 0.22 of the eave apothem and `BuildFinialBase` puts the 宝顶's masonry drum over the
join — which is what a real 攒尖 does anyway.

One methodological note. The converging crown *looked* broken until the preview scene turned on
MSAA: what read as mangled geometry was pure sampling noise. **Judge generated geometry
antialiased**, or aliasing and shape problems are indistinguishable.

### Phase 7 — the roof soffit (望板) — **done, verified**

The roof was see-through from below: standing under an eave you saw sky between the rafters, and at
grazing angles you saw straight through the near slope into the back of the far one.

The obvious guess is reversed winding, and it is wrong — worth recording because the two failure
modes look identical from the one angle you normally check from. `RoofSoffitDiagnose.tscn` buckets
every roof triangle by the sign of its normal's Y:

```
before:  up=14698  down=4571  side=4763     lowest downward face at y=9.25 (the eave line)
```

A flipped normal would make the surface vanish from **above** too, and it does not. And every one of
those 4571 downward faces belonged to a closed solid — brackets, ridge sweeps, the capped 瓦当 and
滴水 — none to the roof field. The boarding was simply a **zero-thickness single-sided surface** at
all five emission sites. Nothing to cull; nothing there at all.

Fixed by giving the boarding depth rather than by disabling backface culling. Culling is free but it
would invert the lighting on every underside, expose the interior of every solid, and fight the
planned NPR shading. `AddRoofPanel` emits the weather face, the soffit under it, and — via the
`ERoofPanelEdges` flags — the rim that closes them at an open edge. That rim is also what stops the
eave reading as a knife edge. 卷棚 is why the flags are a set rather than a bool: its profile runs
eave to eave over the roll, so its last panel's *upper* edge is an eave too.

**The soffit must drop straight down, not along the panel normal.** Offsetting along each panel's
own normal splits the surface at every seam where the slope changes — adjacent panels push their
shared corners in different directions — and the first render came back laced with hairline cracks
across the whole underside. A vertical drop gives every panel the same displacement, so shared
corners stay shared and the soffit is watertight. It also agrees with 举架, whose depths are reckoned
vertically. The thickness measured perpendicular to the slope then varies with pitch, which is
correct for a deck on rafters anyway.

Cost is almost nothing: **+24 triangles on 硬山 to +1680 on 盔顶**, 0.2%–7%. The boarding was never
the expensive part — the tile skin is.

### Phase 7b — the 山花 hole — **done, verified**

A second, unrelated defect on the same elevation: seen from the gable end, roughly half the 山花
tympanum was missing, which reads as a hole in the side of the roof.

This one **was** a facing problem, unlike the soffit. `MeshAccumulator::AddPolygon` fan-triangulated
and decided its winding **once**, from the first three points — valid only for a convex outline. The
山花 outline is not convex: the 举架 curve caves in, so once the fan passes the ridge apex the
triangles' own orientation reverses relative to that single reference and they get culled. Fixed by
orienting each fan triangle against the supplied normal individually, and by skipping degenerate
slivers, of which the duplicated apex vertex produces two. The triangle count fell by exactly 2,
which is the confirmation.

The diagnostic worth keeping is `RoofCullDiagnose.tscn`: it renders one camera twice, as shipped and
with `CULL_DISABLED`, and diffs the frames. Whatever appears only in the two-sided pass is a
one-sided surface seen from behind; whatever is absent from both was never generated. That separates
the two failure modes in a single run — and it is how this was found, because a boundary-edge count
could not. The model is legitimately a stack of open shells rather than a watertight solid, so
3.7%–8.4% of roof edges are open *by design* and the count carries no signal. The frame diff went
10.91% → 7.50%, and the residue is all thin-geometry aliasing at grazing angles, no solid regions.

### Not yet done


- **多重檐** (Fig 3 shows top/middle/bottom roofs). This is the one remaining item from the paper
  and it is not just another roof type: it needs the *body* to become multi-storey — a wall band and
  bracket band per tier, with a roof generated at each level on a shrinking footprint. That is a
  change to `BuildBody`, so it was scoped out rather than rushed.
- **No rafters (椽) under the eave.** The soffit is a plain 望板 deck; a real overhang shows 椽 and
  飞椽 crossing it, which is the most-looked-at surface on the whole building at ground level.
  Cheap to add — one small box per rafter along the eave — but not needed for the roof to read solid.
- **No along-slope articulation.** Tile *rows* overlapping down the slope were costed at ~2.5× the
  skin and deferred; with 瓦当 and 滴水 in place the roof reads as tiled without them, so this is now
  a close-range-LOD question rather than a gap.
